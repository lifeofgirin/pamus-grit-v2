...
export default function Home(){
 ...
 const[user,setUser]=useState<User|null>(null),[view,setView]=useState<"daily"|"work"|"weekly"|"dayGrid"|"monthly"|"students"|"classes"|"teachers"|"classWeekly">("daily"),[date,setDate]=useState(""),[lessons,setLessons]=useState<Lesson[]>([]),[events,setEvents]=useState<EventRow[]>([]);
 ...
 const[dayGridDate,setDayGridDate]=useState("");
 ...
 const selectedDayGrid=useMemo(()=>{
   const target=dayGridDate||weekDays?.[0]?.date||"";
...
if(cleanedDays.length&&!dayGridDate){
 ...
 async function switchView(v:"daily"|"work"|"weekly"|"dayGrid"|"monthly"|"students"|"classes"|"teachers"|"classWeekly"){
  setView(v);
...
<button className="logout-button" onClick={logout}>로그아웃</button></div></header>
  ...
  :view==='work'?<section className="schedule-panel work-panel">
   ...
   <div className="print-toolbar no-print">
     ...
     <button type="button" className="print-action-button" onClick={printCurrentSchedule}>{tr('A4 인쇄','A4 Print')}</button>
   </div>
